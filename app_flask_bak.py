...
def load_db():
    ...
    # Robust read with retry logic to avoid race conditions under concurrency
    import time
    for i in range(5):
        try:
            ...
            print(f"⚠️ Warning: Reading database failed (attempt {i+1}/5): {str(e)}")
            time.sleep(0.05 * (i + 1))
            
    # If read fails after retries, raise error to prevent overwriting database with blank template
    raise RuntimeError("Database file is corrupted or locked. Access denied to prevent data loss.")

def save_db(data):
    temp_file = DB_FILE + '.tmp'
    try:
        # Atomic write pattern: write to a temp file then atomically swap it
...
def not_found(e):
    ...
    return render_template('login.html', error_message="Halaman tidak ditemukan. Kembali ke halaman login.")
...
def handle_exception(e):
    ...
    return render_template('login.html', error_message="Terjadi kesalahan server internal. Silakan hubungi administrator.")
